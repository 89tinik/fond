 ...
 */
class m241203_155739_insert_data_field_table_for_archeology extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        $contestId = (new \yii\db\Query())
            ->select('id')
            ->from('{{%contests}}')
            ->where(['name' => 'Археология'])
            ->scalar();

        if (!$contestId) {
            echo "Конкурс 'Археология' не найден.\n";
            return;
        }
        ...
        $fieldsData = [
            ...
            $sections[2] => [
                ...
                ['name' => 'UF_CRM_DEAL_1690807683809', 'label' => 'Полное название археологического памятника, на базе которого планируется проведение проекта', 'type' => 'text'],
                ['name' => 'UF_CRM_DEAL_1690807948706', 'label' => 'Реквизиты открытого листа за предыдущий год (номер, кем и когда выдан)', 'type' => 'text'],
                ...
                ['name' => 'UF_CRM_DEAL_1690811299964', 'label' => 'Начало реализации проекта', 'type' => 'text'],
                ['name' => 'UF_CRM_DEAL_1690808002849', 'label' => 'Окончание реализации проекта', 'type' => 'text'],
                ...
                ['name' => 'UF_CRM_DEAL_1690808108539', 'label' => 'Сумма запрашиваемой финансовой поддержки', 'type' => 'text', 'b24entity' => 'company'],
            ],
            $sections[3] => [
            ...
            $sections[4] => [
                ...
                ['name' => 'UF_CRM_COMPANY_1690811028100', 'label' => 'География деятельности', 'type' => 'text', 'b24entity' => 'company'],
                ['name' => 'UF_CRM_COMPANY_1691475122802', 'label' => 'Основные проекты соискателя по направлению конкурса за последние два года', 'type' => 'text', 'b24entity' => 'company'],
            ],
            $sections[5] => [
